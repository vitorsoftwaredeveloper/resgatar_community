import { APIGatewayEvent, APIGatewayProxyResult } from "aws-lambda";
import { sendErrorResponse, sendSuccessResponse } from "../../utils/http";
import { decodeToken } from "../../utils/helper";
import { listDonationsService } from "../../services/donations/listDonations";
import { STATUS_CODE } from "../../constants";

export const execute = async (
  event: APIGatewayEvent,
): Promise<APIGatewayProxyResult> => {
  console.log("IN - listDonations");

  try {
    const credentials = decodeToken(event.headers.authorization as string);

    const yearParam = event.queryStringParameters?.year;
    const year = yearParam ? Number(yearParam) : new Date().getFullYear();

    const donations = await listDonationsService(credentials.sub, year);

    return sendSuccessResponse("Donations listed", STATUS_CODE.SUCCESS, donations);
  } catch (error) {
    console.log({ error });
    return sendErrorResponse(error);
  } finally {
    console.log("OUT - listDonations");
  }
};
